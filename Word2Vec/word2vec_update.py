...
class word2vec:
    ...
    def train(self, num_epochs=2, window_size=2, num_negative_samples=2,
              learning_rate=0.05, nll_update_iter=10000, max_steps=-1, synonyms_file=None):
        '''
        ...
        if synonyms_file is not None:
            with open('synonyms.txt', 'r') as f:
                while (f.readline()):
                    ...
                    tokens = self.tokenize(data)
                    for i in tokens:
                        if self.word_to_index.get(i):
                            synonyms_dict[self.word_to_index[i]] = tokens
                        # else:
                        #     synonyms_dict[self.word_to_index['<UNK>']] = tokens
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for input_id_index in tqdm(
                ...
                if cur_context_word_id == self.word_to_index['<UNK>']:
                    continue
                if self.full_token_sequence_as_ids[input_id_index] in synonyms_dict:
                    synonyms_word = random.choice(synonyms_dict[self.full_token_sequence_as_ids[input_id_index]])
                    if self.word_to_index.get(synonyms_word):
                        cur_context_word_id = self.word_to_index[synonyms_word]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--synonyms_file', type=str, default='synonyms.txt',
                        help="The file name for the synonyms file to use in training")
    ...
    parser.add_argument('--output_file', type=str, default='output_update.txt',
    ...
    model.train(nll_update_iter=args.nll_update_iter, max_steps=args.max_steps,
                num_epochs=args.num_epochs, synonyms_file=args.synonyms_file)
