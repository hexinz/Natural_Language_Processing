...
class word2vec:
    ...
    def train(self, num_epochs=2, window_size=2, num_negative_samples=2,
        ...
        for epoch in range(1, num_epochs+1):
            ...
            for input_id_index in tqdm(range(window_size,len(self.full_token_sequence_as_ids)-window_size)): # 2 ~ n-2
...
def predict_and_backprop(W, C, learning_rate, center_id, context_id,
    ...
    h = W[center_id]
    v_c = C[:, context_id] # output layer's activation for context word
    ...
    # you want to use it
    v_neg = C[:, negative_sample_ids] # output layer's activation for negative samples
    ...
    t = np.zeros(len(negative_sample_ids) + 1)
    t[0] = 1
    # posi_pred = sigmoid(np.dot(h, v_c))
    # nega_pred = sigmoid(np.dot(h, v_neg))
    # print(posi_pred, nega_pred)

    # Step 4: Compute the negative log likelihood
    E = - np.log(sigmoid(np.dot(h, v_c))) - np.sum(np.log(sigmoid(-np.dot(h, v_neg))))

    # Step 5: Update the negative sample vectors to push their dot product with the
    # center word's vector closer to zero.
    C[:, context_id] = C[:, context_id] - learning_rate * (sigmoid(np.dot(v_c, h)) - 1) * h

    v_c = np.reshape(v_c, (np.shape(v_neg)[0], 1))
    v_prime_old = np.concatenate((v_c, v_neg), axis=1)

    for j in negative_sample_ids:
        #  compute the gradient descent for the error from the output layer to the hidden layer
        C[:, j] = C[:, j] - learning_rate * sigmoid(np.dot(C[:, j], h)) * h
    ...
    gradient = np.zeros(np.shape(v_neg)[0])
    for j in range(len(negative_sample_ids) + 1):
        gradient += (sigmoid(np.dot(v_prime_old[:, j], h)) - t[j]) * v_prime_old[:, j]
    # gradient = np.sum((sigmoid(np.dot(v_prime_old, h)) - t) * v_prime_old)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nll_update_iter', type=int, default=10000,
